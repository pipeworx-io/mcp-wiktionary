...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wiktionary');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    case 'definition':
      return wikiRest(lang, `/page/definition/${encodeURIComponent(word)}`, word);
    case 'summary':
      return wikiRest(lang, `/page/summary/${encodeURIComponent(word)}`, word);
    case 'search':
...
async function wikiRest(lang: string, path: string, word: string) {
  assertLang(lang);
  const url = `https://${lang}.wiktionary.org/api/rest_v1${path}`;
  const res = await pwFetch(url, {
    headers: { Accept: 'application/json', 'User-Agent': UA },
  });
  // A word Wiktionary has no page for is a no-result, not a defect — this used
  // to throw "page not found" and book as a Pipeworx error. `result_count: 0`
  // + the empty array is what makes the gateway book it as `empty`.
  if (res.status === 404) {
    return {
      word,
      found: false,
      result_count: 0,
      results: [],
      hint: `No ${lang}.wiktionary.org entry titled "${word}". Titles are case-sensitive ("Paris" and "paris" are different pages) — use the wiktionary search tool to find the exact entry title.`,
    };
  }
  if (!res.ok) {
