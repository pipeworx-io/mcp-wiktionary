...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/definition \
  -H 'Content-Type: application/json' \
  -d '{"word":"serendipity"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/definition`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "wiktionary": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-wiktionary"]
    }
  }
}
```
